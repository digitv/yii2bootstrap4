...
class ActiveField extends \yii\widgets\ActiveField
{
    ...
    public function checkbox($options = [], $enclosedByLabel = true)
    {
        if ($enclosedByLabel) {
            ...
            $this->labelOptions['class'] = 'form-check-label';
    ...
    public function checkboxList($items, $options = [])
    {
        if ($this->inline) {
            ...
            Html::addCssClass($options['itemOptions'], 'form-check-input');
            Html::addCssClass($options['itemOptions']['labelOptions'], 'form-check-label');
        }
        if (!isset($options['item'])) {
            ...
            $options['item'] = function ($index, $label, $name, $checked, $value) use ($itemOptions, $isInline) {
                ...
                return $isInline
                    ...
                    : '<div class="form-check">' . Html::checkbox($name, $checked, $options) . '</div>';
            };
        }
    ...
    protected function createLayoutConfig($instanceConfig)
    {
        ...
        if ($layout === 'horizontal') {
            ...
            $config['errorOptions'] = ['class' => 'form-text text-danger ' . $cssClasses['error'], 'tag' => 'small'];
            $config['hintOptions'] = ['class' => 'form-text ' . $cssClasses['hint'], 'tag' => 'small'];
            Html::addCssClass($config['options'], 'row');
